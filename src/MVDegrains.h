...
static void Degrain_sse2(uint8_t *pDst, int nDstPitch, const uint8_t *pSrc, int nSrcPitch, const uint8_t **pRefs, const int *nRefPitches, int WSrc, const int *WRefs) {
    ...
    //TODO: Can we stick this in a loop???
    //Seriously consider doing so, as it might simplify the heck out of this code.
    //Create a branch that uses a loop and test the speed.
    //Test at different block sizes, 8, 16, 32, 64 should be good,
    //Maybe test at different resolutions to be sure.
    //Likely conduct the test in the AVX2 code first, or force the path for each.
    if (radius > 1) {
